...
const SERVICES = [
  ...
  {
    name: "payments",
    script: "server/payment-server.js",
    args: [],
    healthUrl: "http://127.0.0.1:4178/api/health",
    label: "Payment API",
  },
  {
    name: "assets",
    script: "server/asset-server.js",
    args: [],
    healthUrl: "http://127.0.0.1:4179/api/health",
    label: "Asset API",
  },
  {
    name: "timeline",
    script: "server/timeline-server.js",
    args: [],
  ...
  {
    name: "invoices",
    script: "server/invoice-server.js",
    args: [],
    healthUrl: "http://127.0.0.1:4181/api/health",
    label: "Invoice API",
  },
  {
    name: "quotes",
    script: "server/quote-server.js",
    args: [],
...
function spawnService(service) {
  ...
  child.on("spawn", () => {
    console.log(
      `Chrysalis ${service.name} process started.`
    );
  });
  ...
  child.on("exit", (code, signal) => {
    ...
    console.error(
      `Chrysalis ${service.name} service stopped unexpectedly (code=${
        code ?? "null"
      }, signal=${signal ?? "none"}).`
    );
...
async function main() {
  ...
  for (const service of SERVICES) {
    console.log(
      `  ${service.label}: ${service.healthUrl.replace(
        "/api/health",
        ""
      )}`
    );
