...
function listAssets(database, { clientId = "", jobId = "" } = {}) {
  if (clientId && jobId) {
    return database.prepare(
      "SELECT * FROM assets WHERE client_id = ? AND job_id = ? ORDER BY date DESC, created_at DESC, id DESC"
    ).all(clientId, jobId).map(rowToAsset);
  }
  if (clientId) {
    return database.prepare(
      "SELECT * FROM assets WHERE client_id = ? ORDER BY date DESC, created_at DESC, id DESC"
    ).all(clientId).map(rowToAsset);
  }
  if (jobId) {
    return database.prepare(
      "SELECT * FROM assets WHERE job_id = ? ORDER BY date DESC, created_at DESC, id DESC"
    ).all(jobId).map(rowToAsset);
  }
  return database.prepare(
    "SELECT * FROM assets ORDER BY date DESC, created_at DESC, id DESC"
  ).all().map(rowToAsset);
}
...
function saveAsset(database, input, existing = null) {
  ...
  const asset = {
    ...
    kind: String(source.kind || (source.category === "Photo" ? "photo" : "file")),
    caption: String(source.caption || source.name || source.title || source.fileName || "Untitled asset"),
    date: String(source.date || now),
    url: String(source.url || source.dataUrl || ""),
  ...
  database.prepare(`
    INSERT INTO assets
      (id, client_id, job_id, kind, caption, date, url, file_path, created_at, updated_at, data_json)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function deleteAsset(database, id) {
...
function createServerHandler(database) {
  return createServer(async (request, response) => {
    ...
    const first = match?.[1] ? decodeURIComponent(match[1]) : null;
    const second = match?.[2] ? decodeURIComponent(match[2]) : null;

    try {
      if (request.method === "GET" && url.pathname === "/api/health") {
        sendJson(response, 200, { ok: true, service: "assets", databasePath: DB_PATH });
        return;
      }
      ...
      if (request.method === "GET" && first === "client" && second) {
        sendJson(response, 200, { ok: true, assets: listAssets(database, { clientId: second }) });
        return;
      }

      if (request.method === "GET" && first === "job" && second) {
        sendJson(response, 200, { ok: true, assets: listAssets(database, { jobId: second }) });
        return;
      }

      if (request.method === "GET" && first) {
        sendJson(response, 200, { ok: true, asset: getAsset(database, first) });
        return;
      }

      if (request.method === "POST" && !first) {
        const body = await readJsonBody(request);
        sendJson(response, 201, { ok: true, asset: saveAsset(database, body.asset || body) });
        return;
      }

      if ((request.method === "POST" || request.method === "PUT") && first) {
        const body = await readJsonBody(request);
        const incoming = body.asset || body;
        const existing = getAsset(database, first);
        sendJson(response, 200, {
          ok: true,
          asset: saveAsset(database, { ...incoming, id: existing?.id || incoming.id || first }, existing),
        });
        return;
      }

      if (request.method === "DELETE" && first) {
        sendJson(response, 200, { ok: true, deleted: deleteAsset(database, first) });
        return;
      }
      ...
      console.error(error);
      sendJson(response, /not found/i.test(message) ? 404 : 400, { ok: false, error: message });
    }
  });
}
...
function main() {
  ...
  const server = createServerHandler(database);
  server.listen(PORT, "127.0.0.1", () => {
    console.log(`Chrysalis Asset API listening on http://127.0.0.1:${PORT}`);
  });
