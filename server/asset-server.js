...
function saveAsset(database, input, existing = null) {
  ...
  const asset = {
    ...
    caption: String(source.caption || source.fileName || "Untitled asset"),
  ...
  if (!asset.jobId) throw new Error("An asset must have a jobId.");
  ...
  ON CONFLICT(id) DO UPDATE SET
    ...
    asset.id, asset.clientId, asset.jobId, asset.kind, asset.caption,
    asset.date, asset.url, asset.filePath, asset.createdAt,
    asset.updatedAt, JSON.stringify(asset)
  );
  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && first === "client" && second) {
        sendJson(response, 200, { ok: true, assets: listAssets(database, "client", second) });
        return;
      }

      if (request.method === "GET" && first === "job" && second) {
        sendJson(response, 200, { ok: true, assets: listAssets(database, "job", second) });
        return;
      }

      if (request.method === "GET" && first && !second) {
        const asset = getAsset(database, first);
      ...
      if ((request.method === "POST" || request.method === "PUT") && first) {
        ...
        const asset = saveAsset(database, { ...incoming, id: existing?.id || incoming.id || first }, existing);
        sendJson(response, 200, { ok: true, asset });
        return;
      }
