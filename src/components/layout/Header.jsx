...
function getTemporaryBackupInfo(backup) {
  ...
  const createdAt = backup.createdAt || backup.modifiedAt || null;
  ...
  const expiryTime =
    createdTime + TEMP_BACKUP_MAX_AGE_DAYS * 24 * 60 * 60 * 1000;
  ...
  return {
    ...
    warning: remainingMs > 0 && remainingDays <= TEMP_BACKUP_WARNING_DAYS,
  };
}
...
function appointmentDateKey(value) {
  ...
  const australianMatch = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (australianMatch) {
...
function jobOutstanding(job) {
  const explicit = Number(job?.outstanding ?? job?.balance);
...
export default function Header({
    ...
    async function checkTemporaryBackups() {
      try {
        ...
        const warnings = backups
          ...
          .filter((backup) => backup && !backup.expired && backup.warning)
          .sort((a, b) => a.remainingDays - b.remainingDays);

        if (!cancelled) {
          setBackupWarning(warnings.length > 0 ? warnings[0] : null);
        }
      } catch (error) {
        console.warn("Unable to check temporary backup notifications.", error);
      }
    }

    checkTemporaryBackups();

    const interval = window.setInterval(checkTemporaryBackups, 60 * 60 * 1000);
  ...
  useEffect(() => {
    if (!backupWarning) setNotificationDismissed(false);
  }, [backupWarning]);
  ...
  const warningDays = backupWarning?.remainingDays || 0;
