...
function getNavigationAction(question) {
  ...
  if (text.includes("overdue") || text.includes("garment") || text.includes("production")) {
    return { page: "garments", label: text.includes("overdue") ? "Overdue" : "All Garments", labelText: text.includes("overdue") ? "Open overdue garments" : "Open garment production" };
  }
...
export default function BuddiAssistant({ open, onClose, currentPage, clients = [], jobs = [], onNavigate }) {
  ...
  async function answerQuestion(rawQuestion) {
    ...
    try {
      ...
      setConversation((items) => [...items, { question: trimmedQuestion, answer: `I couldn't reach the live Buddi service. ${error.message || "Please check that the server is running and configured correctly."}`, action: null }]);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <aside aria-label="Buddi assistant" style={{ position: "fixed", right: 24, bottom: 94, zIndex: 1199, width: "min(420px, calc(100vw - 32px))", maxHeight: "min(720px, calc(100vh - 130px))", overflowY: "auto", background: "#FFFFFF", border: "1px solid #E8E8E8", borderRadius: 20, boxShadow: "0 22px 70px rgba(47,58,63,.25)", color: "#2F3A3F" }}>
      ...
      <header style={{ padding: "20px 20px 18px", borderBottom: "1px solid #ECE8E5", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg, #FFF8FA 0%, #FFFFFF 65%)" }}>
        <div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 11, fontWeight: 900, letterSpacing: ".1em", textTransform: "none", fontVariant: "normal", fontVariantCaps: "normal", color: "#8B1E3F" }}><span aria-hidden="true">🦋</span>BizziBuddi Assistant</div>
