...
function ThinkingIndicator() {
  return (
    ...
    >
      ...
      <span className="buddi-thinking-dots" aria-hidden="true">
        <span />
        <span />
        <span />
      </span>
...
export default function BuddiAssistant({
  open,
  onClose,
  currentPage,
  clients = [],
    ...
    const context = {
      ...
      jobs: jobs.map((job) => ({
        ...
        clientName: getClientName(
          clients.find(
            (client) => client?.id === job?.clientId || client?.id === job?.client_id
          )
        ),
      ...
      calculatedInsights: {
        overdueJobs: insights.overdueJobs.length,
        dueThisWeek: insights.dueThisWeek.length,
        activeJobs: insights.activeJobs.length,
        outstanding: insights.outstanding,
      },
    };
    ...
    try {
      const [response] = await Promise.all([
        fetch("/api/donna/chat", {
          ...
          body: JSON.stringify({ message: trimmedQuestion, context }),
        }),
        minimumThinkingTime,
      ]);
      ...
      if (!response.ok) {
        throw new Error(payload?.error || "Buddi could not answer right now.");
      }
      ...
      setConversation((items) => [
        ...items,
        {
          ...
          answer: payload.answer || "I couldn't generate an answer.",
        },
      ]);
    } catch (error) {
      ...
      setConversation((items) => [
        ...
        {
          question: trimmedQuestion,
          answer: `I couldn't reach the live Buddi service. ${error.message || "Please check that the server is running and configured correctly."}`,
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <aside
      ...
      style={{
        ...
        maxHeight: "min(720px, calc(100vh - 130px))",
        overflowY: "auto",
        background: "#FFFFFF",
        border: "1px solid #E8E8E8",
        borderRadius: 20,
        boxShadow: "0 22px 70px rgba(47,58,63,.25)",
        color: "#2F3A3F",
      }}
    >
      <style>{`
        @keyframes buddiDotPulse {
      ...
      <header style={{ padding: "20px 20px 18px", borderBottom: "1px solid #ECE8E5", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(135deg, #FFF8FA 0%, #FFFFFF 65%)" }}>
        <div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 11, fontWeight: 900, letterSpacing: ".1em", textTransform: "uppercase", color: "#8B1E3F" }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <form onSubmit={(event) => { event.preventDefault(); answerQuestion(question); }} style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          ...
          <button type="submit" disabled={!question.trim() || isLoading} style={{ border: "none", borderRadius: 10, padding: "0 14px", background: isLoading ? "#C8AAB4" : "#8B1E3F", color: "#FFFFFF", fontWeight: 800, cursor: isLoading ? "wait" : "pointer" }}>
            Ask
          </button>
        </form>
        ...
        <div style={{ marginTop: 18, paddingTop: 12, borderTop: "1px solid #ECE8E5", fontSize: 12, color: "#8A9295" }}>
          Buddi uses your current workspace information to help answer your questions.
        </div>
      </div>
    </aside>
  );
