    ...
    <main style={dashboardStyle}>
      ...
      <section style={metricsStyle} aria-label="Dashboard summary">
        ...
        <Metric
          ...
          action={activeJobs.length ? "View all" : null}
          onClick={activeJobs.length ? () => onSelectJob?.(activeJobs[0]) : undefined}
        />
        <Metric
          icon="💰"
          label="Outstanding Payments"
          value={formatCurrency(outstanding)}
          action={outstanding > 0 ? "View payments" : null}
          onClick={
            outstanding > 0
              ? () => onSelectJob?.(allJobs.find((job) => getOutstanding(job) > 0))
              : undefined
          }
          last
        />
    ...
    <section style={attentionStyle}>
      ...
      <div style={attentionBodyStyle}>
        <div style={attentionRowStyle}>
          <span style={attentionDotStyle} aria-hidden="true">
            •
          </span>
...
function getOutstanding(job) {
  ...
  if (Array.isArray(job.invoices) && job.invoices.length) {
    return Math.max(
      job.invoices.reduce((sum, invoice) => sum + Number(invoice.balance ?? 0), 0),
      0
    );
  }
...
const mainGridStyle = {
  display: "grid",
  gridTemplateColumns: "minmax(0, 1.02fr) minmax(380px, .98fr)",
  gap: 10,
  alignItems: "start",
};
...
const attentionStyle = {
  ...
  borderRadius: 8,
  overflow: "hidden",
  boxShadow: "0 1px 3px rgba(31,41,51,.02)",
};
