...
export default function QuickJobFinder({
  jobs = [],
  clients = [],
  onSelectJob,
}) {
  ...
  const clientNames = useMemo(() => {
    ...
    clients.forEach((client) => {
      map.set(String(client.id), getClientName(client));
    });
  ...
  const recentJobs = useMemo(() => {
    return [...jobs]
      .filter((job) => !["Archived"].includes(String(job.status || "").trim()))
      .sort((a, b) => {
  ...
  const results = useMemo(() => {
    ...
    return jobs
      .filter((job) => {
        const clientName =
          job.clientName ||
          clientNames.get(String(job.clientId)) ||
          job.client ||
          "";
  ...
  return (
    <section style={finderStyle} aria-label="Find a job">
      ...
      <div style={listHeaderStyle}>
        <span>
          {query.trim() ? "Search Results" : "Recent Jobs"}
        </span>
