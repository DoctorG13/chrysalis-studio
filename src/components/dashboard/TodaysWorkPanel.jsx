  ...
  return (
    <div style={panelStyle}>
      {priorities.length > 0 && <section style={attentionStyle}>
        <div style={attentionHeadingStyle}>⚠️ Needs Attention</div>
        {priorities.slice(0, 3).map((job) => {
          const client = clients.find((item) => String(item.id) === String(job.clientId));
          const priority = getPriority(job);
          return <div key={job.id ?? job.reference ?? job.name} style={attentionRowStyle}>
            <span style={{ color: priority.color, fontSize: 15 }}>•</span>
            <div style={rowContentStyle}><strong>{job.reference || job.name || job.title || "Job"}</strong><small>{getClientName(client)} · {job.nextAction || priority.label}</small></div>
...
function TodayRow({ icon, title, subtitle, onClick }) {
  return <div style={todayRowStyle}><span style={todayIconStyle}>{icon}</span><div style={rowContentStyle}><strong>{title}</strong><small>{subtitle}</small></div>{onClick && <OpenButton onClick={onClick} />}</div>;
}

function OpenButton({ onClick }) {
  return <button type="button" aria-label="Open" onClick={onClick} style={openButtonStyle} onMouseEnter={(event) => { event.currentTarget.style.background = "#8B1E3F"; event.currentTarget.style.color = "#FFFFFF"; event.currentTarget.style.boxShadow = "0 3px 8px rgba(139,30,63,.18)"; event.currentTarget.style.transform = "translateY(-1px)"; }} onMouseLeave={(event) => { event.currentTarget.style.background = "#FFFFFF"; event.currentTarget.style.color = "#8B1E3F"; event.currentTarget.style.boxShadow = "0 1px 2px rgba(0,0,0,.04)"; event.currentTarget.style.transform = "translateY(0)"; }}>Open <span aria-hidden="true">→</span></button>;
}
...
function getPriority(job) { if (job.overdue) return { label: "Overdue", color: "#B42318" }; if (job.dueToday) return { label: "Due today", color: "#B54708" }; return { label: "Needs attention", color: "#9A6700" }; }
...
const panelStyle = { padding: "0 20px 18px" };
const todayListStyle = { display: "flex", flexDirection: "column" };
const todayRowStyle = { display: "flex", alignItems: "center", gap: 17, minHeight: 72, padding: "13px 0", borderBottom: "1px solid #E8EAED" };
const todayIconStyle = { width: 34, textAlign: "center", fontSize: 25, flexShrink: 0 };
const rowContentStyle = { display: "flex", flexDirection: "column", gap: 4, minWidth: 0, flex: 1 };
const openButtonStyle = { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, flexShrink: 0, minWidth: 100, height: 42, padding: "0 18px", border: "1px solid #C96A83", borderRadius: 999, background: "#FFFFFF", color: "#8B1E3F", fontSize: 13, fontWeight: 700, lineHeight: 1, cursor: "pointer", boxShadow: "0 1px 2px rgba(31,41,51,.04)", transition: "background 160ms ease, color 160ms ease, box-shadow 160ms ease, transform 160ms ease" };
const attentionStyle = { margin: "0 0 14px", border: "1px solid #E8C7CF", borderRadius: 10, overflow: "hidden", background: "#FFF8FA" };
const attentionHeadingStyle = { padding: "13px 16px", color: "#8B1E3F", fontSize: 14, fontWeight: 800, borderBottom: "1px solid #F0DCE1" };
const attentionRowStyle = { display: "flex", alignItems: "center", gap: 10, padding: "12px 16px" };
